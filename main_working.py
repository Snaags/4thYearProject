...
for i in X:
	sample.append(i)
	lable.append(math.sin((i/180)*math.pi))



sample = np.asarray(lable)

lable = np.asarray(lable[1:])

#scaler = MinMaxScaler(feature_range=(-1, 1))	#scale data
#sample = scaler.fit_transform(sample.reshape(-1, 1))
#lable = scaler.fit_transform(lable.reshape(-1, 1))

sample=GroupData(sample,1,1) 
...
X_train, X_test = test_train_split(sample,0.50) #Training data from 80% of the total data set

y_train, y_test = test_train_split(lable,0.50) #Training data from 80% of the total data set
x_real = X_test
...
model = LSTMModel(input_dim = 1, hidden_dim =50, output_dim=1, layer_dim = 1, seq = 1)
model.cuda()
...
#optimiser = torch.optim.ASGD(model.parameters(), lr=0.1)
...
plt.plot(realy, label=("Data days ahead"))
plt.legend()
plt.show()
