...
if __name__ == "__main__":
	...
	while run == True:
		...
		if flag == False:
			...
			os.mkdir("Graphs/"+str(counter)+"-"+filename)			
			for i in os.listdir("Graphs"):
				x = i.split("/")[-1]
				if x[-4:] != ".pdf":
					continue	

				os.replace("Graphs/"+i, "Graphs/"+str(counter)+"-"+filename+"/"+x)
				print("Moved files to", filename)
		else:
			flag = False
		searchSpace = []
		while True:
			filename = os.listdir("StockData")[random.randint(0,len(os.listdir("StockData"))-1)]
			file = pandas.read_csv(path+"/StockData/"+filename)
			if len(file) != 2769:
				continue
